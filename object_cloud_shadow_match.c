...
void Union(cloud_node* node1, cloud_node* node2) {
...
)
{
    ...
    for (row = 0; row < nrows; row++)
    {
     ...
     else
     {
        ...
        if (obj_num == NULL || cloud == NULL)
        {
            sprintf (errstr, "Allocating memory for obj_num/cloud");
            ERROR (errstr, "cloud/shadow match");
            return -1;
        }
        ...
        for (row = 0; row < nrows; row++)
        {
           for (col = 0; col <ncols; col++)
           {
               ...
               cloud[row][col].rank = 0;
           }
        }

        /* Labeling the cloud pixels */
        label(cloud_mask, nrows, ncols, cloud, obj_num);
